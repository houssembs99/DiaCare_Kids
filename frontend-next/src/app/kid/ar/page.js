...
export default function ARPage() {
  ...
  return (
    <div id="ar-ui-overlay" className="fixed inset-0 bg-transparent overflow-hidden font-sans select-none z-[9999]">
      
      {/* 1. SCÈNE AR (TOUJOURS EN ARRIÈRE-PLAN - PLEIN ÉCRAN) */}
      <div className="absolute inset-0 z-0">
        <ARScene animationName={animation} />
      </div>

      {/* 2. INTERFACE DE CONTRÔLE FLOTTANTE (FORCÉE DEVANT) */}
      <div className="absolute inset-0 z-[100] flex flex-col pointer-events-none">
        
        {/* Barre du Haut (Navbar + Interaction) - Bloquée en haut */}
        <div className="w-full bg-gradient-to-b from-[#0b1b2b] to-transparent pb-10 pointer-events-auto shrink-0">
            {/* Identity line */}
            <div className="h-[70px] pt-10 px-6 flex items-center justify-center">
                <div className="w-24 h-1.5 bg-white/10 rounded-full" />
            </div>

            {/* BARRE D'INTERACTION COMPACTE */}
            <div className="px-4">
                <div className="h-[85px] px-4 flex items-center justify-between bg-[#0b1b2b]/80 backdrop-blur-xl border border-white/10 rounded-[30px] shadow-2xl gap-2">
                    {/* GAUCHE : Quitter, Sport, AR */}
                    ...
                    <div className="flex-1 flex justify-center px-2">
                        <div className={`flex items-center justify-center border-2 px-4 py-2 rounded-2xl min-w-[70px] bg-black/40 ${
                            currentStatus === 'perfect' ? 'border-green-500/50 text-green-400' : 
                            currentStatus === 'hypo' ? 'border-red-500/50 text-red-400' : 'border-orange-500/50 text-orange-400'
                        }`}>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                        <button onClick={() => handleItemClick('apple')} className="p-2 bg-green-500/20 border border-green-500/30 rounded-lg text-green-400"><Apple size={16} /></button>
                        <button onClick={() => handleItemClick('insulin')} className="p-2 bg-blue-500/20 border border-blue-500/30 rounded-lg text-blue-400"><Syringe size={16} /></button>
                        <button onClick={() => handleItemClick('candy')} className="p-2 bg-pink-500/20 border border-pink-500/30 rounded-lg text-pink-400"><Candy size={16} /></button>
                        <div className="hidden xs:block text-right ml-1 border-l border-white/10 pl-2">
                            <h1 className="text-white font-black italic uppercase tracking-tighter text-[9px] leading-none">
